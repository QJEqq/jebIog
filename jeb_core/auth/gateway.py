        ...
        if response.status_code == 200:
            ...
            if res_data.get('status') == 'ok':
                ...
                if gen_code:
                    ...
                    print("\n=== ДЕБАГ ОТПРАВКИ НА СЕРВЕРЕ ===")
                    print(f"Сгенерирован ключ кэша: 'valid_code_for_{phone_str}'")
                    print(f"Записали туда пин-код: '{gen_code}' (Тип: {type(gen_code)})")
                    print("=================================\n")
...
def checkVerificationStatus(phone_number, user_code):
    try:
        ...
        print("\n=== ДЕБАГ ВЕРИФИКАЦИИ НА СЕРВЕРЕ ===")
        print(f"1. Ищем в кэше по ключу: '{cache_key}'")
        print(f"2. Что достали из кэша (saved_code): '{saved_code}' (Тип: {type(saved_code)})")
        print(f"3. Что ввёл пользователь (user_code): '{user_code}' (Тип: {type(user_code)})")
        print("====================================\n")
